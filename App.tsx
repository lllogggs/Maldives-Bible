              ...
              <div className="space-y-3 sm:space-y-4">
                <header className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-500 sm:text-sm sm:leading-6 sm:text-slate-600">
                      <span className="sm:hidden">예산·이동·객실 조건으로 171곳을 빠르게 찾아보세요.</span>
                      <span className="hidden sm:inline">171개 리조트를 예산, 이동수단, 객실 유형, 개인풀과 수중환경 기준으로 비교해 보세요.</span>
                    </p>
                  </div>
                  <DesktopViewingTip />
                </header>
                <div className="sticky top-[57px] z-30 -mx-4 grid grid-cols-1 gap-2 border-y border-slate-200 bg-[#f6f8f7]/95 px-4 py-2.5 backdrop-blur lg:static lg:mx-0 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-6 lg:border-x-0 lg:border-t-0 lg:bg-transparent lg:px-0 lg:pt-0 lg:backdrop-blur-none">
                  <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center lg:col-start-2">
                    <div className="flex min-w-0 flex-1 gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex h-10 shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 shadow-sm shadow-slate-900/5 hover:border-slate-300 hover:bg-slate-50 active:bg-slate-100 lg:hidden"
                      >
                        <FilterIcon className="h-5 w-5" />
                        <span>필터{getActiveFilterCount(filters) > 0 ? ` ${getActiveFilterCount(filters)}` : ''}</span>
                      </button>
                    </div>
                    <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 sm:flex sm:justify-end">
                      ...
                      <span className="shrink-0 whitespace-nowrap text-xs font-semibold text-slate-500 lg:hidden">
                        {displayedResorts.length}곳
                      </span>
                    </div>
