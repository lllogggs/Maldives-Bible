...
const TrendChart: React.FC<{ points: AnalyticsTrendPoint[] }> = ({ points }) => {
  ...
  const maxClicks = Math.max(...points.map(point => point.agencyClicks), 1);
  ...
  const clickY = (value: number) => padding.top + chartHeight - (value / maxClicks) * (chartHeight * 0.72);
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap gap-4 text-xs font-bold text-slate-500">
        ...
        <span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-sm bg-amber-400" />여행사 클릭</span>
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full overflow-visible" role="img" aria-label="기간별 방문 세션과 여행사 클릭 추이">
        <defs>
        ...
        {points.map((point, index) => {
          ...
          const top = clickY(point.agencyClicks);
          return (
            <rect
              key={`bar-${point.date}`}
              ...
              rx={barWidth / 2}
              fill="#fbbf24"
              opacity={point.agencyClicks > 0 ? 0.9 : 0.15}
            />
          );
        })}
      ...
      <main className="mx-auto max-w-[1480px] space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 font-brand-heading text-2xl text-slate-950 sm:text-3xl">방문자가 여행사까지 가고 있는지</h2>
          </div>
        ...
        <section className="grid gap-4 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,.55fr)]">
              ...
              <div className="flex items-start justify-between gap-5">
                <div>
                  <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-extrabold text-teal-50 ring-1 ring-white/15">
                    <span className="h-2 w-2 rounded-full bg-emerald-300" />{trackingLabel}
                  </span>
                  <p className="mt-5 text-sm font-bold text-teal-100/80">여행사 버튼 클릭</p>
                  <div className="mt-2 flex flex-wrap items-end gap-x-4 gap-y-2">
                    <strong className="font-brand-heading text-5xl leading-none sm:text-6xl">{formatNumber(data.agencies.clicks)}</strong>
                    <span className="pb-1 text-sm font-bold text-teal-100">회</span>
                  </div>
                </div>
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15">
                  <MousePointerClick className="h-6 w-6 text-amber-300" />
                </span>
              </div>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-black/15 p-4 ring-1 ring-white/10">
                  <p className="text-[11px] font-bold text-teal-100/70">클릭률</p>
                  <p className="mt-1 font-brand-heading text-2xl">{formatPercent(data.agencies.clickRate)}</p>
                </div>
                <div className="rounded-2xl bg-black/15 p-4 ring-1 ring-white/10">
                  <p className="text-[11px] font-bold text-teal-100/70">버튼 노출</p>
                  <p className="mt-1 font-brand-heading text-2xl">{formatNumber(data.agencies.impressions)}</p>
                </div>
                <div className="col-span-2 rounded-2xl bg-black/15 p-4 ring-1 ring-white/10 sm:col-span-1">
                  <p className="text-[11px] font-bold text-teal-100/70">클릭 사용자</p>
                  <p className="mt-1 font-brand-heading text-2xl">{data.agencies.clickUsers === null ? '—' : formatNumber(data.agencies.clickUsers)}</p>
                </div>
              </div>
            </div>
          </article>
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-1">
            <KpiCard label="방문 세션" value={formatNumber(current.sessions)} detail="사이트 방문 묶음" change={change.sessions} icon={<Activity className="h-5 w-5" />} />
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-900/[0.035] sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <SectionTitle eyebrow="Trend" title="방문과 여행사 클릭 추이" description="세션 흐름과 가장 중요한 외부 이동을 같은 날짜에서 비교합니다." />
        ...
        <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,.85fr)]">
          <article className="min-w-0 rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/[0.035]">
            <div className="border-b border-slate-100 p-5 sm:p-6"><SectionTitle eyebrow="Acquisition" title="어디서 들어왔나" description="유입경로별 참여와 여행사 클릭 전환을 함께 봅니다." /></div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] text-left">
                <thead className="bg-slate-50/80 text-[11px] font-extrabold text-slate-500">
                  <tr><th className="px-5 py-3">유입경로</th><th className="px-3 py-3 text-right">세션</th><th className="px-3 py-3 text-right">참여율</th><th className="px-3 py-3 text-right">평균 체류</th><th className="px-5 py-3 text-right">여행사 클릭률</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
